...
| C Building/houses | C0 build-mode UX contract; C1 placement/rotation/dismantle polish; C2 shelter/room validation; C3 readable house-kit assets; C4 utility integrations; C5 functional-home playtest; C6 wall/shell socket contract | B1, H0 | C5 is proven for the first hearth loop; C1 has placement rotation/pack proof; C2 now has a single-room enclosure and comfort-readability contract around the claimed bedroll; C2/C3 has relocation, code-owned house-kit sockets, socket-specific snap-preview silhouettes, and the first normalized Kiln door/window/roof skins as decorative art only; C6 now has a second procedural/code-owned wall-shell slice: floor foundations, full wall panels, integrated wall-door panels, integrated wall-window panels, corners, half rails, and roof joins are craftable/placeable, previewed, saved, and browser-proofed. Remaining C6 work is true edge-addressed sockets, multi-piece-per-tile building, collision polish, broader room shapes, and shared-scale decorative skins |
...
| C6 wall/shell socket contract | Building sim, renderer, and asset-fit lanes | Second slice passing | `floorFoundation`, `wallPanel`, `wallDoorPanel`, `wallWindowPanel`, `wallCorner`, `wallHalfRail`, and `roofJoin` are code-owned/craftable sockets with renderer silhouettes, shelter contribution rules, save/build coverage, and browser proof. Door/window/roof GLBs remain decorative skins or inserts. Remaining C6 work must define true edge-addressed sockets, multi-piece-per-tile snapping, collision, broader room shapes, and shared-scale decorative skins before craftable house shells can be considered polished |
...
| H5 | The approved Kiln pack replaces janky procedural visuals across the world | Active; K1/K2/K5 repeated-family slices, K6 animated-creature skin slice, K6T targetability, K6R roaming, K9 fish, K11 sky life, and second C6 procedural wall-shell slice passing | Main orchestrator plus asset-runtime, batching/perf, animation, UX, and proof lanes | H4, A4 | R4, R5 | `docs/kiln-pack-adoption-plan.md`; `npm run proof:k1-resource-drops`; `npm run proof:k2-domain-resources`; `npm run proof:k5-trees`; `npm run proof:k6-creatures`; `npm run proof:k7-native-targeting`; `npm run proof:k6r-roaming`; `npm run proof:k9-fish-visuals`; `npm run proof:k11-sky-life`; `npm run proof:c6-wall-shells`; current evidence proves waystone, door/window/roof skins, wood/ore pickup skins, all 12 domain resource node skins, four tree/shrub vegetation skins, all nine native-creature skins, native creature targeting/build blockers, sparse creature roaming, five fish/driftjelly bodies, four bird bodies, and code-owned foundations/walls/integrated openings/corners/rails/roof joins | Continue with C6 edge sockets, multi-piece snapping/collision, K3 home props, K4 waterline utilities, K7 wonders, and remaining modular kits |
...
| C6 | A functional house has real wall and shell sockets | Second slice passing | Building sim/renderer plus asset-fit lanes | C2/C3, H5 | R1, R2, R4, R5 | `floorFoundation`, `wallPanel`, `wallDoorPanel`, `wallWindowPanel`, `wallCorner`, `wallHalfRail`, and `roofJoin`; `test/structures.test.ts`; `test/buildCommands.test.ts`; `test/structureRenderer.test.ts`; `npm run proof:c6-wall-shells`; `output/playwright/c6-wall-shells/proof.json` | Add true edge-addressed sockets, multi-piece-per-tile snapping/collision, broader room shapes, and shared-scale decorative skins after proof |
