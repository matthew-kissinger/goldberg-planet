...
| B3/C5 hearth contract | Main implementation with frontier-review lane | Done for functional-home contract | `npm run proof:hearth-contract` seeds the real hearth site, proves missing requirements, builds the functional home, completes site work, opens 12 terrain cells across 3 underpass tiles, spends a root-cellar provision, grants trail focus, and captures PC/laptop/tablet/phone/gamepad-active screenshots |
...
| R1 Sim/save gate | Focused sim tests pass and debug hooks expose durable state without hiding progression in UI-only code | `npm test -- journal` covers crowded ready-site journal priority; `completeSiteWork` debug proof returns the same threshold carve result that gameplay triggers | Passing for the hearth-contract slice | Save import/export compatibility still needs a broader pentagon-site regression suite |
| R2 Playability gate | Real player loop changes state without debug-only shortcuts | `npm run proof:hearth-contract` proves the functional-home/site-work loop changes site readiness, completion, threshold state, terrain cells, root-cellar provisions, and survival trail focus | Passing for this slice | Future site kinds and modular house-kit snapping need the same loop proof |
| R3 Cross-device gate | PC/laptop, touch, and gamepad evidence for affected verbs | Hearth-contract proof covers desktop, 1366x720 laptop, tablet touch portrait, phone touch portrait, and a gamepad-active profile; panel ownership proof still covers blocked-motion panel parity | Passing for the current affected verbs | Landscape tablet/phone and exact gamepad structure-placement/use paths remain later all-verb parity work |
...
| R5 Performance/release gate | Typecheck, unit suite, build, browser screenshots, console check, canvas proof, bundle/audio budgets | `npm run proof:kiln-assets` validates 61 committed GLBs; `npm run proof:route-markers` proves committed model requests; `npm run proof:hearth-contract` records five screenshots, no page/console errors, and functional-home terrain/progression evidence | Passing for the promoted Kiln pack, first runtime pilot, and hearth-contract proof | Production large-chunk warning, SFX loudness/provenance cleanup, and future family-specific scale/collision proofs remain known follow-ups |
