...
| E Food/farming/fishing | E0 crop/fish tuning; E1 crop variety; E2 traps/nets loop; E3 cooking/preservation effects; E4 ecology-to-route balance | B2, C4, D4 | First E4 route-pressure slice is closed: ready fish traps and shore nets, plus an active fish run, provide capped staged waterline resupply in Route Slate prep; route-local consumption and richer food variety remain |
...
| E4 ecology-to-route balance | Main implementation with proof/docs reviewer lane | Done for capped waterline resupply | `npm run proof:e4-ecology-route` proves unready traps/nets leave packed food missing, ready trap + net resupply flips the food check, visible Route Slate target rows keep the waterline detail on desktop and phone touch, and screenshots record no page/console errors |
...
| E4 | Ecology prep changes expedition readiness without becoming free global food | Done for capped waterline resupply | Main implementation plus proof/docs reviewer lane | B2, C4, D4 | R1, R2, R3, R5 | `src/sim/navigation.ts`; `src/main.ts`; `test/navigation.test.ts`; `scripts/proof-e4-ecology-route.mjs`; `output/playwright/e4-ecology-route/proof.json`; desktop and phone-touch screenshots | Add route-adjacent staging/consumption, stricter spatial checks, more food variety, and broader laptop/tablet/gamepad proof before treating this as final travel logistics |
...
| R1 Sim/save gate | Focused sim tests pass and debug hooks expose durable state without hiding progression in UI-only code | `npm test -- landmarks navigation caveMouths` covers threshold terrain specs and high-clearance arch readiness; `npm test -- test/navigation.test.ts` covers capped waterline route resupply and the no-ready-gear negative control | Passing for the E4 capped-resupply slice | Save import/export compatibility still needs a broader pentagon-site and route-food regression suite |
| R2 Playability gate | Real player loop changes state without debug-only shortcuts | `npm run proof:hearth-contract` proves the functional-home/site-work loop; `npm run proof:d2-caves` proves First Hearth and Deep Bell terrain spaces; `npm run proof:e4-ecology-route` proves visible Route Slate food prep changes from missing to expedition-ready after ready trap/net state | Passing for this slice | Future site kinds, modular house-kit snapping, and route-local food consumption need the same loop proof |
| R3 Cross-device gate | PC/laptop, touch, and gamepad evidence for affected verbs | Hearth-contract proof covers desktop, 1366x720 laptop, tablet touch portrait, phone touch portrait, and gamepad-active profile; D2 proof adds desktop and phone-touch hearth/bell screenshots; E4 proof adds desktop and phone-touch Route Slate screenshots | Passing for the current affected verbs only | Landscape tablet/phone, laptop-height E4 proof, and exact gamepad structure-placement/use paths remain later all-verb parity work |
...
| R5 Performance/release gate | Typecheck, unit suite, build, browser screenshots, console check, canvas proof, bundle/audio budgets | `npm run proof:kiln-assets` validates 61 committed GLBs; `npm run proof:route-markers` proves committed model requests; `npm run proof:hearth-contract`, `npm run proof:d2-caves`, and `npm run proof:e4-ecology-route` record screenshots, screenshot/canvas pixels, no page/console errors, and terrain/progression/route-prep evidence | Passing for the promoted Kiln pack, first runtime pilot, hearth contract, first D2 threshold-space proof, and first E4 capped-resupply proof | Production large-chunk warning, SFX loudness/provenance cleanup, and future family-specific scale/collision proofs remain known follow-ups |
...
expeditions, with an active fish run adding a small confidence bonus. The Route Slate
target row keeps that waterline detail visible even when route candidates become
selectable, and `npm run proof:e4-ecology-route` proves the unready-to-ready food-check
transition on desktop and phone touch. The next Cycle 4 work is route-adjacent staging or
consumption, stricter spatial checks, net ecology tuning, deeper food effects, and more
cave/flight expedition food variety.
