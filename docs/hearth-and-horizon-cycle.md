...
| H5 approved Kiln pack adoption | Main orchestrator with asset-runtime, batching/perf, animation, UX, and proof lanes | Active goal-level priority; K1 pickups, K2 resource nodes, K5 trees, and K6 native-creature skins passing | `docs/kiln-pack-adoption-plan.md` makes the 61 ready GLBs the target visual backlog; every asset family must become runtime wired, runtime dressing, regenerated, or explicitly superseded; repeated families must use palette/material reuse, instanced or batched geometry, and distance-gated animation before broad rollout; `npm run proof:k1-resource-drops` proves `drop-wood-logs` and `drop-ore-chunk` load from committed `models/`, batch 5 spawned drops into 5 instanced draw calls, collect into inventory, capture desktop/phone screenshots, and make no `generated/` runtime requests; `npm run proof:k2-domain-resources` proves all 12 `node-*` GLBs load from committed `models/`, batch 36 revealed domain nodes into 33 instanced draw calls, keep pending/fallback at zero, capture desktop/phone screenshots, and make no `generated/` runtime requests; `npm run proof:k5-trees` proves `tree-pine`, `tree-broadleaf`, `tree-dead-snag`, and `tree-shrub` load from committed `models/`, replace procedural chunk tree meshes only after all skins are ready, render 210 resident trees on 11 instanced draw calls, keep cosmetic sway near-only, fell a tree into wood drops, capture desktop/phone screenshots, and make no `generated/` runtime requests; `npm run proof:k6-creatures` proves all nine `creature-*` GLBs load from committed `models/`, require idle/walk clips, distance-gate AnimationMixer bands, keep fallback at zero, preserve tend/ward responses, capture desktop/phone screenshots, and make no `generated/` runtime requests |
...
| H5 | The approved Kiln pack replaces janky procedural visuals across the world | Active; K1/K2/K5 repeated-family slices and K6 animated-creature skin slice passing | Main orchestrator plus asset-runtime, batching/perf, animation, UX, and proof lanes | H4, A4 | R4, R5 | `docs/kiln-pack-adoption-plan.md`; `npm run proof:k1-resource-drops`; `npm run proof:k2-domain-resources`; `npm run proof:k5-trees`; `npm run proof:k6-creatures`; current evidence proves waystone, door/window/roof skins, wood/ore pickup skins, all 12 domain resource node skins, four tree/shrub vegetation skins, and all nine native-creature skins | Continue with home props, waterline utilities, landmarks, remaining modular kits, and the K6 native-life targetability/placement UX gap |
...
| K6 creature animation/targetability audit | H5, R4, R5 | Confirm the safest path for promoted creature GLBs and the playtest-reported native-life input gap | Partially accepted | GLB skinning, required clip checks, distance-gated mixer diagnostics, tests, proof script, and docs are merged; native-life targetability remains open | Sidecar audit confirmed K6 asset loading is separate from the UX bug: native life is currently tile-anchored, hazards can apply stamina/exposure pressure, and pick/mining still targets the hex underneath because there is no native-life ray target or placement blocker yet |
...
| R4 Asset readability gate | Reviewer can name the gameplay noun and likely verb from screenshots | Route markers expose named cave-anchor/waystone glyph roles; D2 proof captures separate First Hearth underpass and Deep Bell chamber screenshots; `npm run proof:character` now captures Soft-Facet Wayfarer desktop/laptop/tablet/phone/gamepad/WebGL screenshots after asserting 46 silhouette/readability parts, 3 prop sockets, 21 action poses, held/stowed prop separation, and plane seated cues; C2 now exposes visible warmth halo, home comfort ring, roof shelter glow, window warm light, and structured comfort diagnostics; C2/C3 exposes door/window/roof socket dimensions, socket-specific snap-preview guides for door openings, window light, and roof caps, and fitted promoted door/window/roof GLB skins with procedural fallback; H5 now makes the remaining approved pack the target visual backlog; K1 adds promoted wood/ore pickup GLB skins; K2 adds all 12 promoted domain-resource node skins while keeping code-owned base/glow overlays; K5 adds four promoted tree/shrub skins while keeping chop/drop authority code-owned; K6 adds all nine promoted creature GLB skins while keeping warning/reward overlays and native-life behavior code-owned | Passing for route-marker glyph dialect, waystone pilot, first threshold-space proof, procedural avatar normal-distance contract, first shelter comfort readability, code-owned house-kit sockets, first normalized house-kit skins, first snap-preview readability, first socket-specific house-kit preview silhouettes, first approved-pack pickup pair, first approved-pack resource-node family, first approved-pack vegetation family, and first approved-pack creature skin family; H5 is active, not passing beyond the wired route/house/pickup/node/tree/creature slugs | Blind human screenshot review, native-life targetability/placement UX, full approved-pack adoption, authored model generation/rigging, two-hand prop polish, remaining modular prop families, and further cave-anchor/waystone/threshold polish remain open |
| R5 Performance/release gate | Typecheck, unit suite, build, browser screenshots, console check, canvas proof, bundle/audio budgets | `npm run proof:kiln-assets` validates 61 committed GLBs; `npm run proof:route-markers` proves committed model requests; `npm run proof:k1-resource-drops` proves committed wood/ore pickup model requests, no generated requests, 5 spawned pickup instances on 5 instanced draw calls, desktop/phone screenshots, and collection into inventory; `npm run proof:k2-domain-resources` proves all 12 committed node model requests, no generated requests, 36 revealed node instances on 33 instanced draw calls, desktop/phone screenshots, and zero pending/fallback; `npm run proof:k5-trees` proves all four committed tree model requests, no generated requests, 210 resident tree instances on 11 instanced draw calls, desktop/phone screenshots, zero pending/fallback, near-only sway, and chopped wood drops; `npm run proof:k6-creatures` proves all nine committed creature model requests, no generated requests, required idle/walk clips, active/low-rate/hidden mixer bands, desktop/phone screenshots, zero fallback, and tend/ward gameplay responses; `npm run proof:character` proves avatar renderer stats, screenshot/canvas pixels, and no page/console errors across six profiles; `npm run proof:hearth-contract`, `npm run proof:d2-caves`, `npm run proof:e4-ecology-route`, `npm run proof:f1-itinerary`, `npm run proof:panel-ownership`, `npm run proof:c1-build-placement`, `npm run proof:c2-room-enclosure`, and `npm run proof:c2-c3-building-snap-grid` record screenshots, screenshot/canvas pixels, no page/console errors, command logs, terrain/progression/route-prep/arrival-consumption/organic route/off-route/build-placement/relocation/enclosure evidence, promoted door/window/roof model responses, fitted-bounds diagnostics, valid/blocked snap-preview screenshots, and desktop/laptop door/window/roof placement-preview guide screenshots | Passing for the promoted Kiln pack, first route-marker pilot, first normalized house-kit family, first K1 instanced pickup pair, first K2 instanced resource-node family, first K5 instanced vegetation family, first K6 animated creature-skin family, avatar fallback proof, hearth contract, first D2 threshold-space proof, E4 route-adjacent plus organic source proof, F1 itinerary controls, shared panel ownership, B1 command boundary, C1 build placement, first C2 room enclosure, first C2/C3 snap-grid relocation/preview, and first socket-specific house-kit preview proof | Production large-chunk warning, SFX loudness/provenance cleanup, landscape-device route-food smoke, native-life targetability/placement UX, and remaining family-specific scale/collision proofs remain known follow-ups |
