...
| E Food/farming/fishing | E0 crop/fish tuning; E1 crop variety; E2 traps/nets loop; E3 cooking/preservation effects; E4 ecology-to-route balance | B2, C4, D4 | E4 now has capped route-adjacent staging: ready fish traps and shore nets count only near the active route corridor, with off-route ready gear visible as ignored prep; route-arrival consumption and richer food variety remain |
...
| E4 ecology-to-route balance | Main implementation with proof/docs reviewer lanes | Done for route-adjacent staging | `npm run proof:e4-ecology-route` proves off-route ready gear is ignored, unready route gear leaves packed food missing, ready route-adjacent trap + net resupply flips the food check, visible Route Slate target rows keep the waterline detail across desktop, laptop, tablet touch, phone touch, and synthetic gamepad, and screenshots record no page/console errors |
...
| E4 | Ecology prep changes expedition readiness without becoming free global food | Done for route-adjacent staging | Main implementation plus proof/docs reviewer lanes | B2, C4, D4 | R1, R2, R3, R5 | `src/sim/navigation.ts`; `src/main.ts`; `test/navigation.test.ts`; `scripts/proof-e4-ecology-route.mjs`; `output/playwright/e4-ecology-route/proof.json`; desktop, laptop, tablet-touch, phone-touch, and gamepad screenshots | Add route-arrival consumption of the eligible staged sources, stronger organic placement/wait/haul proof, more food variety, and landscape-device smoke before treating this as final travel logistics |
...
| R1 Sim/save gate | Focused sim tests pass and debug hooks expose durable state without hiding progression in UI-only code | `npm test -- landmarks navigation caveMouths` covers threshold terrain specs and high-clearance arch readiness; `npm test -- test/navigation.test.ts` covers capped route-adjacent waterline resupply, off-route ignored gear, and the no-ready-gear negative control | Passing for the E4 route-adjacent staging slice | Save import/export compatibility still needs a broader pentagon-site, route-food, and future source-consumption regression suite |
| R2 Playability gate | Real player loop changes state without debug-only shortcuts | `npm run proof:hearth-contract` proves the functional-home/site-work loop; `npm run proof:d2-caves` proves First Hearth and Deep Bell terrain spaces; `npm run proof:e4-ecology-route` proves visible Route Slate food prep changes from off-route ignored/missing to expedition-ready after ready route-adjacent trap/net state | Passing for this slice | Future site kinds, modular house-kit snapping, and route-arrival consumption of staged food sources need the same loop proof |
...
gamepad. The next Cycle 4 work is route-arrival consumption of eligible staged sources,
organic placement/wait/haul proof, net ecology tuning, deeper food effects, and more
