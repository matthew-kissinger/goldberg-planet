...
| E Food/farming/fishing | E0 crop/fish tuning; E1 crop variety; E2 traps/nets loop; E3 cooking/preservation effects; E4 ecology-to-route balance | B2, C4, D4 | E4 now has capped route-adjacent staging plus arrival-spent waterline sources: ready route trap/net prep is counted near the active route corridor, off-route ready gear stays ignored, and route arrival clears the eligible staged trap/net timers; richer food variety remains |
...
| E4 | Ecology prep changes expedition readiness without becoming free global food | Done for route-adjacent staging plus route-arrival consumption | Main implementation plus proof/docs reviewer lanes | B2, C4, D4 | R1, R2, R3, R5 | `src/sim/navigation.ts`; `src/sim/structures.ts`; `src/main.ts`; `test/navigation.test.ts`; `test/structures.test.ts`; `scripts/proof-e4-ecology-route.mjs`; `output/playwright/e4-ecology-route/proof.json`; desktop, laptop, tablet-touch, phone-touch, and gamepad screenshots | Add stronger organic placement/wait/haul proof, more food variety, and landscape-device smoke before treating this as final travel logistics |
...
| R5 Performance/release gate | Typecheck, unit suite, build, browser screenshots, console check, canvas proof, bundle/audio budgets | `npm run proof:kiln-assets` validates 61 committed GLBs; `npm run proof:route-markers` proves committed model requests; `npm run proof:hearth-contract`, `npm run proof:d2-caves`, and `npm run proof:e4-ecology-route` record screenshots, screenshot/canvas pixels, no page/console errors, and terrain/progression/route-prep/arrival-consumption evidence | Passing for the promoted Kiln pack, first runtime pilot, hearth contract, first D2 threshold-space proof, and E4 route-adjacent plus arrival-spent source proof | Production large-chunk warning, SFX loudness/provenance cleanup, organic route-food proof, and family-specific scale/collision proofs remain known follow-ups |
...
set and ready. The next Cycle 4 work is organic placement/wait/haul proof, net ecology
tuning, deeper food effects, and more cave/flight expedition food variety.
