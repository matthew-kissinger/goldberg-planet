...
| R1 Sim/save gate | Focused sim tests pass and debug hooks expose durable state without hiding progression in UI-only code | `npm test -- landmarks navigation caveMouths` covers threshold terrain specs and high-clearance arch readiness; `completeSiteWork` and `inspectThresholdChamber` debug proof return structured terrain/read state | Passing for the D2 threshold-space slice | Save import/export compatibility still needs a broader pentagon-site regression suite |
| R2 Playability gate | Real player loop changes state without debug-only shortcuts | `npm run proof:hearth-contract` proves the functional-home/site-work loop; `npm run proof:d2-caves` proves First Hearth and Deep Bell terrain spaces, Route Slate/Journal threshold reads, and reward state in the running game | Passing for this slice | Future site kinds and modular house-kit snapping need the same loop proof |
| R3 Cross-device gate | PC/laptop, touch, and gamepad evidence for affected verbs | Hearth-contract proof covers desktop, 1366x720 laptop, tablet touch portrait, phone touch portrait, and gamepad-active profile; D2 proof adds desktop and phone-touch hearth/bell screenshots | Passing for the current affected verbs | Landscape tablet/phone and exact gamepad structure-placement/use paths remain later all-verb parity work |
...
| R5 Performance/release gate | Typecheck, unit suite, build, browser screenshots, console check, canvas proof, bundle/audio budgets | `npm run proof:kiln-assets` validates 61 committed GLBs; `npm run proof:route-markers` proves committed model requests; `npm run proof:hearth-contract` and `npm run proof:d2-caves` record screenshots, canvas pixels, no page/console errors, and terrain/progression evidence | Passing for the promoted Kiln pack, first runtime pilot, hearth contract, and first D2 threshold-space proof | Production large-chunk warning, SFX loudness/provenance cleanup, and future family-specific scale/collision proofs remain known follow-ups |
