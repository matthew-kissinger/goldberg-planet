...
bedroll and now extends that proof to a full six-edge single-room wall shell: all six
home-tile boundary edges are covered, perimeter coverage is `1`, browser pixels are
recorded, and the existing weaken/collision regressions still run. `shelterReport` still
derives boundary edges as home-tile spokes to the one-ring boundary, so a real connected
multi-room/foundation-expanded house needs an outer-perimeter room-boundary derivation before
second-room exterior walls can become shelter authority. Do not generate or wire the
shared-scale Kiln house-shell pack as craftable building art until the connected-room
perimeter contract and cleaner beauty/readability screenshots are proven; use procedural
wall-shells as the code-owned collider/snap/shelter authority. The current six-edge
screenshot is proof-grade, not a beauty-grade house shot.
...
| C6 wall/shell socket contract | Building sim, renderer, and asset-fit lanes | Edge-socket, edge-shelter, traversal-collision, serviced-room, and full six-edge single-room perimeter slice passing | `floorFoundation`, `wallPanel`, `wallDoorPanel`, `wallWindowPanel`, `wallCorner`, `wallHalfRail`, and `roofJoin` are code-owned/craftable sockets with renderer silhouettes, shelter contribution rules, save/build coverage, edge-addressed occupancy, same-hex center-plus-edge stacking, duplicate-edge blockers, edge-based shelter coverage diagnostics, wall/window/corner traversal blockers, passable integrated door edges, live player movement collision proof, stale collision clearing after relocation, and browser proof. The current proof also turns a bedroll-centered wall-shell room into a protected, functional `shelter alive` home with `lived-in` comfort, lit campfire warmth, workbench, chest, and bedroll shelter sleep, then verifies all six home-boundary edges can be covered in a single-room shell with perimeter coverage `1`. Door/window/roof GLBs remain decorative skins or inserts. Remaining C6 work must define connected/multi-room outer perimeter shapes and cleaner beauty/readability captures before shared-scale Kiln house-shell skins can be generated or treated as polished craftable house art |
...
| C6 | A functional house has real wall and shell sockets | Functional serviced single-room and full six-edge single-room wall-shell proof passing; true multi-room perimeter still open | Building sim/renderer plus asset-fit lanes | C2/C3, H5 | R1, R2, R4, R5 | `floorFoundation`, `wallPanel`, `wallDoorPanel`, `wallWindowPanel`, `wallCorner`, `wallHalfRail`, and `roofJoin`; `ShelterEnclosureReport.coveredBoundaryEdges`; `structureTraversalBlocker`; `Player.update` collision callback; `test/structures.test.ts`; `test/buildCommands.test.ts`; `test/structureRenderer.test.ts`; `npm run proof:c6-wall-shells`; `output/playwright/c6-wall-shells/proof.json`; `output/playwright/c6-wall-shells/wall-shells-six-edge.png` | Next: implement outer-perimeter boundary edges for connected room interior tiles and capture cleaner beauty/readability proof; only then generate and fit shared-scale decorative house-shell skins |
...
| R5 C6 addendum | Wall-shell edge coverage, collision, serviced-room, and six-edge single-room browser proof | `npm run proof:c6-wall-shells` now records edge-based shelter coverage mode, four-edge weather-safe threshold, full six-edge single-room perimeter coverage, covered/door/window boundary edges, wall/window/corner traversal blockers, passable integrated door edges, live player movement hitting a wall blocker, stale corner collision clearing after relocation, protected/functional/lived-in shelter readback, workbench/chest/campfire service readiness, bedroll shelter sleep, screenshots, and canvas pixel probes | Passing for first C6 functional serviced wall-shell room slice plus full six-edge single-room perimeter proof | Broader/multi-room outer-perimeter solving, cleaner beauty/readability captures, and shared-scale house-shell skins remain |
